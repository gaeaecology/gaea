var BLACKBYTES_ASSET = require('GAEAcore/constants').BLACKBYTES_ASSET;
var balances = require('GAEAcore/balances');
...
function initWallet() {
	...
	function loadCompleteClient(showClient) {
		self._bGAEACoreLoaded = false; //"fix" : Looks like you are loading multiple copies of GAEA core, which is not supported. Running 'npm dedupe' might help.
		...
		angularJs.onload = function() {
			var GAEAJS = document.createElement('script');
			GAEAJS.src = 'GAEA.js';
			body.appendChild(GAEAJS);
			GAEAJS.onload = function() {
